...
def receive_iot_data():
    ...
    try:
        ...
        pump = (
            db.query(DeviceStatus)
            .filter_by(id='water_pump', user_id=user_id)
            .first()
        )

        lights = (
            db.query(DeviceStatus)
            .filter_by(id='grow_lights', user_id=user_id)
            .first()
        )
...
@app.route('/api/latest-data/<user_id>', methods=['GET'])
def get_latest_data(user_id):
    ...
    try:
        latest = (
            ...
            .order_by(PlantData.timestamp.desc())
            .first()
        )
...
def control_device():
    ...
    try:
        device = (
            db.query(DeviceStatus)
            .filter_by(id=device_id, user_id=user_id)
            .first()
        )
