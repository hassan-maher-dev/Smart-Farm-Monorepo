...
void main() async {
  ...
  const String serverBaseUrl =
      'http://a57f0c7a9303740ed945af4905efe5e8-1757069761.us-east-1.elb.amazonaws.com';
  ...
  Future<void> updateDeviceState(String deviceId, bool state) async {
    try {
      await http.post(
          Uri.parse('$serverBaseUrl/api/control'), // 👈 التعديل تم هنا بنجاح
  ...
  Timer.periodic(const Duration(seconds: 2), (timer) async {
    try {
      final response =
          await http.get(Uri.parse('$serverBaseUrl/api/devices/$userId'));
  ...
  Timer.periodic(const Duration(seconds: 5), (timer) async {
    ...
    double uvIndex =
        (lightIntensity == 100.0) ? 5.0 + random.nextDouble() * 3.0 : 0.0;
    ...
    try {
      final response = await http.post(Uri.parse('$serverBaseUrl/api/data'),
          headers: {'Content-Type': 'application/json'},
      ...
      if (response.statusCode == 201) {
        print(
            '✅ [$time] Sent Data -> Soil: ${telemetryData['soil_moisture']}% | Temp: ${telemetryData['temperature']}°C | Pump: ${isPumpOn ? "ON" : "OFF"}');
      } else {
